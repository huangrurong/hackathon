        ...
        withCredentials([
            ...
            timeout(90){
                withEnv(["WORKSPACE=${current_workspace}"]){
                    sh './build-config/jobs/build-docker/build_docker.sh'
                }
            }
            archiveArtifacts 'hackathon_docker_images.tar, build_record'
            stash name: 'docker', includes: 'hackathon_docker_images.tar, build_record'
            env.DOCKER_WORKSPACE="${current_workspace}"
            env.DOCKER_STASH_NAME="docker"
            env.DOCKER_STASH_PATH="hackathon_docker_images.tar"
